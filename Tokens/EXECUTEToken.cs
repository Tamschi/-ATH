...
{
    public class EXECUTEToken : _ATHToken
    {
        public static _ATHToken Read(string source, ref int position)
        {
            if (source.Length > position + "EXECUTE".Length && source.Substring(position, "EXECUTE".Length) == "EXECUTE")
            {
                position += "EXECUTE".Length;
                return new EXECUTEToken();
            }
            else
            {
                throw new TokenException("EXECUTE expected at " + position + ".");
            }
        }
    }
}
